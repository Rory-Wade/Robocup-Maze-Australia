...
#define wallAhead 145.0 //distance ahead when facing a wall 
...
int turnTimer = 0;
  ...
  switch (direction) {
    case LEFT:
      currentDirection -= 10; break;
      
    case RIGHT:
      currentDirection += 10; break;
      
    case BACKWARDS:
      currentDirection += 20; break;
  ...
  float targetAngle = currentDirection * 9.0;
  ...
  switch (int(currentDirection)) {
    case 0:
      log(INFO, "CASE 0" );
      while ((accelX < (360.0 - turnThreshold) && accelX > turnThreshold) && digitalRead(pause_button) == HIGH && turnTimer < 100) {
        turnTimer++;
        updateAccel();
    ...
    default:
      log(INFO, "CASE > 0" );
      while (!(accelX < (targetAngle + turnThreshold) && accelX > (targetAngle - turnThreshold)) && digitalRead(pause_button) == HIGH && turnTimer < 100) {
        turnTimer++;
        updateAccel();
        log(INFO, "Accel: %f", accelX);
...
bool moveTile() {
  ...
  if(!onRamp()){
    ...
    if ((Lidar_Front <= forwardTile && Lidar_Front >= (forwardTile - wallThreshold)) || Lidar_Front <= wallAhead || touchF == LOW) {
      turnRobot(3);
    }
