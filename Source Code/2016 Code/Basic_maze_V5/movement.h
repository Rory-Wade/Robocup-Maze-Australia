...
int turnTimer = 0;
  ...
  switch (int(currentDirection)) {
    case 0:
      log(INFO, "CASE 0" );
      while ((accelX < (360.0 - turnThreshold) && accelX > turnThreshold) && digitalRead(pause_button) == HIGH && turnTimer < 40) {
        turnTimer++;
        updateAccel();
    ...
    default:
      log(INFO, "CASE > 0" );
      while (!(accelX < (targetAngle + turnThreshold) && accelX > (targetAngle - turnThreshold)) && digitalRead(pause_button) == HIGH && turnTimer < 40) {
        turnTimer++;
        updateAccel();
        log(INFO, "Accel: %f", accelX);
...
void touchAvoid() {
  updateTouch();
  log(VERBOSE, "Front touch sensor value is: %d", touchF);
  if (touchF == LOW){
